...
use crate::{QuantumCircuit, QuantumState, MeasurementResult};
...
impl<A: Algorithm> AlgorithmRunner<A> {
    ...
    pub fn measure(&self, qubit: usize) -> Result<bool, String> {
        match &self.circuit {
            Some(circuit) => {
                self.algorithm.measure(circuit, qubit)
            }
            None => Err("No quantum circuit available. Did you run the algorithm first?".to_string())
        }
    }
